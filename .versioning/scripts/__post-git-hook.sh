...
TEX_FILES_CHANGED=$(git diff-tree --name-only -r --no-commit-id HEAD | grep '\.tex$')

if [ "$TEX_FILES_CHANGED" != "" ]; then
    echo "$TEX_FILES_CHANGED" | xargs -n 1 ./runTexMake.sh
fi
...
echo "doing: git commit --amend --no-verify -C HEAD"
...
git commit --amend --no-verify -C HEAD
echo "done: git commit --amend --no-verify -C HEAD"
